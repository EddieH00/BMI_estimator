...
class BMI_Estimator(tf.keras.Model):
    def __init__(self):
        ...
        # Define the loss
        self.loss = tf.losses.absolute_difference(self.Y, self.outputs)
    ...
    def train(self, train_data_loader, epochs, save=False, savePath = ''):
        with tf.Session() as sess:
            sess.run(self.init)

            for epoch in range(epochs):
                total_loss = 0
                for i, (inputs, labels) in enumerate(train_data_loader.get_generator()):
                    _, loss_val = sess.run([self.train_op, self.loss], feed_dict={self.X: inputs, self.Y: labels})
                    total_loss += loss_val
                average_loss = total_loss / train_data_loader.num_batches

                print('Epoch {} Average Loss {}'.format(epoch, average_loss))
                if save == True:
                    if os.path.exists(savePath):
                        epoch_folder = os.path.join(savePath, 'epoch_{}'.format(epoch))
                        if not os.path.exists(epoch_folder):
                            os.makedirs(epoch_folder)
                        self.save(sess, os.path.join(epoch_folder, 'model_weights.ckpt'))
                    else:
